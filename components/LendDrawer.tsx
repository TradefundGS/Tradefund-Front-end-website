...
const formSchema = z.object({
	...
	amount: z.number(),
	preference: z.enum(["Visible", "Hide Name", "Hide Amount", "Anonymous"]),
});

type FormData = z.infer<typeof formSchema>;

const LendDrawer: React.FC<LendDrawerProps> = ({
	projectId,
	open,
	onClose,
	refetch,
	...
	const { handleSubmit, control, setValue } = useForm<FormData>({
		...
		defaultValues: {
			proof: undefined,
			preference: "Visible", // default value
		},
	});
	...
	const { mutate, isPending: isLoading } = useMutation({
		mutationFn: (data: any) =>
			mutateFetcherWithAuthAndImage("/project/invest", data),
		onError: (error: any) => {
	...
	const onSubmit = (values: FormData) => {
		...
		formData.append("preference", values.preference); // ✅ already correct
																...
																<div className="space-y-2">
																	...
																	<label className="inline-flex items-center">
																		<input
																			type="radio"
																			value="Hide Amount"
																			checked={field.value === "Hide Amount"}
																			onChange={field.onChange}
																			className="mr-2"
																		/>
