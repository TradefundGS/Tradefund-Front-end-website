...
import { useProfile, useGetRepaymentDetails } from "@/reactQuery/mutation/home";
...
const LoanDrawer: React.FC<LendDrawerProps> = ({ id, open = false, onClose }) => {
    ...
    const { data: repayment } = useGetRepaymentDetails({ id });
 
 
    const [valueInput,setValueInput]=useState(0)
  
    const repaymentData = repayment?.repayments?.[0];
    const { data:bankData, isError:bankisError, error:bankError } = useProfile();
    const [proof, setProof] = useState<File | null>(null);
    const [isSuccess, setIsSuccess] = useState(false);

    const { mutate, isPending: isLoading } = useMutation({
        mutationFn: (data: any) => mutateFetcherWithAuthAndImage("/pay_repayment", data),
        onError: (error) => {
    ...
    useEffect(()=>{
        if(repaymentData?.amount_with_interest){
            setValueInput(repaymentData?.amount_with_interest)
        }
      },[repaymentData?.amount_with_interest])
                                                ...
                                                <div className="space-y-6">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="mt-4 space-y-2 text-sm text-gray-700">
                                                            <p>
                                                                <strong>Account Holder Name:</strong> {bankData?.branch_name || "N/A"}
                                                            </p>
                                                            <p>
                                                                <strong>Account Number:</strong> {bankData?.account_number || "N/A"}
                                                            </p>
                                                            <p>
                                                                <strong>Swift Code:</strong> {bankData?.swift_code || "N/A"}
                                                            </p>
                                                            <p>
                                                                <strong>Bank Name:</strong> {bankData?.bank_name || "N/A"}
                                                            </p>
                                                        </div>
                                                    </div>
